<template>
  <div class="swiper-component" @touchstart="onTouchStart" @touchend="onTouchEnd" :style="{height:height}">
    <ul :style="[ulStyle, swiperStyle]">
      <li v-for="(item, ind) in imageList" :key="ind" :style="{width: itemWidth + 'px'}" :class="[index===ind?'active':'', effect]" @click="handleClick(item)">
        <img :src="item.img">
      </li>
    </ul>
    <div v-show="options.showDots" class="swiper-dots">
      <div v-for="(item, ind) in imageList" :key="ind" class="dots-item" :class="{active:index===ind}"></div>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    calcWidth() {
      ...
      this.$nextTick(function() {
        ...
        this.handleType();
        var length = this.imageList.length; // 获取列表的个数
